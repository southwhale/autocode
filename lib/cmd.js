...
function checkEnv() {
	...
	if (!fs.existsSync(cmd)) {
		console.warn('No such file %s', cmd);
		return;
	}
	...
	if (stats.uid !== uid) {
		console.warn('Recommended using `homebrew`' +
			' install `nodejs`, otherwise you may encounter' +
			' some weird problems.');
	}

	if (process.env.SUDO_UID) {
		console.warn('Never using root to run `autocode`, ' +
...
function leaveRootMode() {
	if (process.env.SUDO_GID) {
		try {
			...
			console.warn('setgid failed, msg = %s', ex.toString());
		}
	}
	...
	if (process.env.SUDO_UID) {
		try {
			...
			console.warn('setuid failed, msg = %s', ex.toString());
		}
	}
}
